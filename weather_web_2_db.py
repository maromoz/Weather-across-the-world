...
def update_attributes(city, country_initials):
    ...
    while not response and attempt < 3:
        ...
        try:
            ...
            if e.code == 404:
                ...
                continue
    if not response:
        print ("could not update %s, skipping" % city)
        return

    if country_initials in country_dict:
        country_name = country_dict[country_initials]
        ...
        country_dict[country_initials] = country_name
    ...
    with connection.cursor() as cursor:
        try:
            ...
            if count_country == 0:
                cursor.execute("insert into countries (name, initials,last_updated) values (%s, %s, %s)",
                               (country_name, country_initials, last_updated))

            cursor.execute(
                "insert into cities (name, temperature, humidity, pressure, temperature_min, temperature_max, country_initials,last_updated) values (%s,%s,%s,%s,%s,%s,%s,%s) on duplicate key update last_updated = values(last_updated);",
                (city, temp, humidity, pressure, temp_min, temp_max, country_initials, last_updated))
